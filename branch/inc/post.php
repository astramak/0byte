...
if (isset($_GET['tag'])) {
	...
	$tpl->assign("tag_use",1);
	$tpl->assign("tag_use_name",gtext($_GET['tag']));
...
if (isset($_GET['fnd'])) {
	...
	$tpl->assign("fnd_use",1);
	$tpl->assign("fnd_name",gtext($_GET['fnd']));
...
//		else {$rt=0;}
		$tpl->assign("blog_use",1);
		$tpl->assign("blog",array("name"=>$blg->name,"id"=>$blg->id,"about"=>$blg->about,"av_use"=>$av_use,
		"av"=>$blg->av,"rate"=>$blg->rate(),"in"=>$in));
...
//			$ssa="";
			$tpl->assign("auth_post",1);
			$avuse=0;
			if (strlen($alien->av)>2) {
				$avuse=1;
//				$ssa= "<img  src='res.php?t=av&img=".$alien->av."' style='float:left;' alt='".$alien->login."' />";
			}
			$tpl->assign("auth_pst",array("login"=>$alien->login,"av"=>$alien->av,"rate"=>$alien->rate(),"avuse"=>$avuse));
//			$rate=$alien->rate();
//			if ($rate>0) {$rt="<span class='rp'>".$rate."</span>";}
