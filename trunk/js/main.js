...
for (var i=0; i<(document.links.length); i++)
{
 ...
 if (document.links[i].href.indexOf('twork.php?wt=ratec')+1)
	 ...
	 if (document.links[i].href.indexOf('twork.php?wt=pmdel')+1)
		  {
...
if (tp!='lg' && tp!='ulg') {
  ...
  //     alert(jr.err);
     // alert(1);
     //alert(xrt.indexOf("err"));

if (jr) {
	if (!jr.err) {
if (tp=="p") {
	if (parseInt(jr.rate)>0) {jr.rate="<span class='rp'>"+jr.rate+"</span>";}
	else if (parseInt(jr.rate)<0) {jr.rate="<span class='rm'>"+jr.rate+"</span>";}
	document.getElementById("rp"+jr.id).innerHTML=jr.rate;
	 } else 
if (tp=="u") {
	if (parseInt(jr.rate)>0) {jr.rate="<span class='rp'>"+jr.rate+"</span>";}
	else if (parseInt(jr.rate)<0) {jr.rate="<span class='rm'>"+jr.rate+"</span>";}
	document.getElementById("ru"+jr.id).innerHTML=jr.rate; }  else 
if (tp=="b") {
	if (parseInt(jr.rate)>0) {jr.rate="<span class='rp'>"+jr.rate+"</span>";}
	else if (parseInt(jr.rate)<0) {jr.rate="<span class='rm'>"+jr.rate+"</span>";}
	document.getElementById("rb"+jr.id).innerHTML=jr.rate; } else 
	if (tp=="c") {
		if (parseInt(jr.rate)>0) {jr.rate="<span class='rp'>"+jr.rate+"</span>";}
	else if (parseInt(jr.rate)<0) {jr.rate="<span class='rm'>"+jr.rate+"</span>";}
		document.getElementById("rc"+jr.id).innerHTML=jr.rate; } else 
		if (tp=="m") {document.getElementById("cm"+jr.id).innerHTML+=jr.text; }
		else if (tp=='f') {
...
xmlhttp.onreadystatechange = function() { r_s(xmlhttp.readyState,xmlhttp.status,xmlhttp.responseText,tp); };
xmlhttp.send(null);
}
function s_c(a,id) {
 // alert(a.text.value);
...
xmlhttp.onreadystatechange = function() { r_s(xmlhttp.readyState,xmlhttp.status,xmlhttp.responseText,"newcom"); };
