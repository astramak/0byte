...
function imga(i,fr,el) {
	a_cr();
	alt='';
	if (i.alt.value!='') {alt="="+i.alt.value;}
...
function scm(a,id,lvl) {
	...
	xmlhttp.onreadystatechange = function() { r_s(xmlhttp.readyState,xmlhttp.status,xmlhttp.responseText,"scom"); };
...
var selcol;
function mkcol() {
	if (parseInt(document.getElementsByName("r")[0].value)>255) {document.getElementsByName("r")[0].value=255;}
	if (parseInt(document.getElementsByName("g")[0].value)>255) {document.getElementsByName("g")[0].value=255;}
	if (parseInt(document.getElementsByName("b")[0].value)>255) {document.getElementsByName("b")[0].value=255;}
	if (parseInt(document.getElementsByName("r")[0].value)<0) {document.getElementsByName("r")[0].value=0;}
	if (parseInt(document.getElementsByName("g")[0].value)<0) {document.getElementsByName("g")[0].value=0;}
	if (parseInt(document.getElementsByName("b")[0].value)<0) {document.getElementsByName("b")[0].value=0;}
	selcol="#"+
...
function col(c,w) {
	if (w==0) {
		if (parseInt(document.getElementsByName(c)[0].value)>0) {
			 ...
			 if (parseInt(document.getElementsByName(c)[0].value)<0) {document.getElementsByName(c)[0].value=0;}
		}
	} else {
		if (parseInt(document.getElementsByName(c)[0].value)<255) {
			 document.getElementsByName(c)[0].value=parseInt(document.getElementsByName(c)[0].value)+10;
			 if (parseInt(document.getElementsByName(c)[0].value)>255) {document.getElementsByName(c)[0].value=255;}
		}
	}
	mkcol();
}
...
function prv() {
	...
	if (xmlhttp.readyState==4) { document.getElementById('prv').innerHTML="<h2>Предпросмотр  <a href='javascript:klprv()'>X</a></h2>"+xmlhttp.responseText; } };
	xmlhttp.send("text="+encodeURIComponent(a.text.value));
	document.getElementById('prv').style.display='block';
}
function ce(e){
...
function upd_com() {
	...
	com_ls=0;
	var xmlhttp = getXmlHttp();
	xmlhttp.open('GET', "js/r_get.php?last_com_id="+last_com_id+"&pid="+pid, true);
	...
	xmlhttp.onreadystatechange = function() { if (xmlhttp.readyState==4) {var resp=eval('('+xmlhttp.responseText+')');
	num_com=0;
...
function prw_com(txt,cid) {
	...
	xmlhttp.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
	xmlhttp.onreadystatechange = function() { 
	if (xmlhttp.readyState==4) { document.getElementById('cprv'+cid).innerHTML="<h4>Предпросмотр  <a href='javascript:klcprv("+cid+")'>X</a></h4>"+xmlhttp.responseText; } };
