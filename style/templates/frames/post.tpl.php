<div class='<?php if ($first) { echo 'fst_top'; } else {echo 'top';} ?>'><span class='date'>
<?php echo $date; ?>
</span>
<?php if (!$not_in_blog) {?>
    <a class='auth' href='user/<?php echo $author; ?>/'><?php echo $author; ?></a>
<?php }?></div>
<div class='text'><h2 class='title' <?php if ($blocked) { ?>style='color:red'<?php } ?>>
<?php
if ($type=='link') {?>
    <a href='<?php echo $link; ?>' rel='nofollow' class='blog'>
    <img class='auth' src='style/lnk.gif' style='height: 20px;' /></a>
<?php } else if ($type=='translate') {?>
